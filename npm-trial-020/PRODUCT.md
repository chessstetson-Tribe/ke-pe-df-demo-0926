...
**Also built — A2a** (flexible precedent search, sitting between A1 and A2): the central "matching term sheets to precedent" throughline, made explicit and flexible rather than only auto-suggested. Facet dropdowns (sponsor tier, industry, lender, deal-size bucket) and a natural-language box both call the *same* scoring detector (`detectors/searchPrecedents.ts`) as A0's auto-match — one capability, three entry points (auto-suggest on open, facets, natural language), not three separate search features. Reachable from A0 ("Search the full precedent bank instead") and from A2's own shortlist-narrowing panel ("Open flexible search"). `matchPrecedents.ts` itself was also extended to score on sponsor tier and facility-size proximity, not just industry/covenant, so A0's "why this was suggested" reasoning is richer even before a user ever opens A2a.
