...
export function buildSystemPrompt(state: DemoState): string {
  ...
  return `You are deal counsel for a direct lender's legal team, working ${deal.name}. Answer ONLY from the file below; never invent facts; cite sources inline as [S#]. Be concise (2-5 sentences), lead with the bottom line.
